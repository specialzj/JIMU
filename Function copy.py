...
class Function:
    ...
    async def Move_posetuning_list(self, pose_tuple, work_name, index,arm="all"):
        """
        说明:
            movel可以每次移动一个坐标
            根据输入pose,示教法移动多组坐标。
        ...
        """
        # from IPython import embed; embed()
        pose_tuple = self.pose_deal(pose_tuple,work_name,index)
        move_pose_tuple = {}
        config_list = self.init["Pose_List"][work_name]
        euler = self.init["ROBOT1"]["Euler"]
        move_new_pose_tuple={}
        move_new_pose_tuple['left_arm'] = []
        move_new_pose_tuple['right_arm'] = []  

        def calculate_new_pose(pose_cfg, robot_cfg, arm):
            move_arm = ''
            """计算新的姿态坐标"""
            if arm == 'left_arm' or arm == 'right_arm' and robot_cfg[4] == "same" and robot_cfg[4] != "dif":
                move_arm =arm
            elif robot_cfg[4] == "dif" and arm == 'left_arm':
                move_arm = "right_arm"
            elif robot_cfg[4] == "dif" and arm == 'right_arm':
                move_arm = "left_arm"
            elif robot_cfg[4] == "left_arm" or robot_cfg[4] == "right_arm":
                move_arm = robot_cfg[4]
            if len(config_list[pose_cfg][0])==6:  # ["teach_pose"] != 0
                return self.posetuning_calculate(
                    pose_tuple[move_arm][robot_cfg[0]],
                    config_list["apose"][robot_cfg[0]],
                    config_list[pose_cfg][0],
                    euler
                )
            return pose_tuple[move_arm][robot_cfg[0]] 
        ...
        def apply_offset(pose, offset, offset_type="base"):
            ...
            pose = [round(num, 3 - len(str(int(num)))) if num != 0 else 0 for num in pose]
            return pose
        ...
        async def execute_move(task_type, pose, arm, robot_cfg):
            ...
            if task_type == "MoveL":
                move_pose_tuple[target_arm] = pose
                if self.robot =="W1":
                    self.w1_instruction.get_move_pose(move_pose_tuple)
                    task = asyncio.create_task(self.w1_instruction.async_MoveL(move_pose_tuple,arm))
                if self.robot =="AGILE":
                    task = asyncio.create_task(self.agile_instruction.async_MoveL(pose,arm))
            
            elif task_type == "MoveJ":
                if self.robot =="W1":
                    task = asyncio.create_task(self.w1_instruction.async_MoveJ(pose,arm))
                if self.robot =="AGILE":
                    task = asyncio.create_task(self.agile_instruction.async_MoveJ(pose,arm))
            return await task
        ...
        for pose_cfg in config_list:
            if "tpose" in pose_cfg or "jpose" in pose_cfg:
                ...
                # 计算目标姿态
                if task_type == "MoveL":
                    new_pose = calculate_new_pose(pose_cfg, robot_cfg, arm)
                    ...
                    if robot_cfg[4] == "same" and not pose_name.startswith(("l_", "r_")):
                        if arm == "left_arm":
                            pose_name = "l_" + pose_name
                        elif arm == "right_arm":       
                            pose_name = "r_" + pose_name
                    new_pose = (
                        self.init["Pose_List"]["basic_pose"][pose_name]
                        if pose_name in self.init["Pose_List"]["basic_pose"]
                        else config_list[pose_cfg][0])
                ...
                if self.log_msg[f"{task_type} success"][1] == 1:
                    logging.debug(self.log_msg[f"{task_type} success"][0].format(robot_cfg[4] if arm == 'all' else arm, new_pose))
                success = await execute_move(task_type, new_pose, arm, robot_cfg)

                # 记录夹爪状态
                # if self.log_msg["gripper_status"][1] == 1:
                #     logging.info(self.log_msg["gripper_status"][0].format(robot_cfg[1], robot_cfg[2], robot_cfg[3]))

                # 等待时间和夹爪操作
                time.sleep(robot_cfg[1])
                if self.robot =="W1":
                    self.w1_instruction.gripper("l", robot_cfg[2])
                if self.robot =="AGILE" and robot_cfg[1] != 0 and robot_cfg[3] != 0: 
                    if arm == 'left_arm': robot_name = "ROBOT1" 
                    else: robot_name = "ROBOT2"
                    u = self.init[robot_name]["USB"]
                    if isinstance(robot_cfg[2], list):
                        angle = robot_cfg[2][1]
                        gripper_status = robot_cfg[2][0]
                    else:
                        angle = u["angle"]-360
                        gripper_status = robot_cfg[2]
                    if robot_cfg[2] == 1: str1 = "open"
                    elif robot_cfg[2] == 0: str1 = "close"
                    self.agile_instruction.usb_grap(gripper_status, angle, robot_name)
                    if self.log_msg["gripper_status"][1]==1: logging.info(self.log_msg["gripper_status"][0].format(robot_cfg[1], str1, robot_cfg[3]))
                ...
                # 判断是否需要等待任务完成
                if len(config_list[pose_cfg]) == 3 and task_type == "MoveJ":
